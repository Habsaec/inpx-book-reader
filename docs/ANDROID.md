...
### Совместимость позиции чтения

Android-клиент отправляет `positionVersion: 4`, `baseRevision` и точный FB2-якорь
(`sectionIndex`, `textOffset`, `textQuote`, `textSectionLength`) в
`POST /api/books/:id/position`. При первом открытии локальной позиции версии
ниже 4 миграция учитывает формат: FB2/FBZ полностью сбрасывает устаревшие
координаты, а EPUB сохраняет только CFI и заново синхронизирует его по CAS.
При восстановлении FB2 точный текстовый якорь проверяется по разделу,
смещению и цитате; затем используются fallback по `fraction` и `fb2Href`.
