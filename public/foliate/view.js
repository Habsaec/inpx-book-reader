...
export class View extends HTMLElement {
    ...
    async open(book) {
        ...
        style.textContent = ':host { display: block; width: 100%; height: 100%; } foliate-paginator, foliate-fxl { display: block; width: 100%; height: 100%; }'
    ...
    #handleLinks(doc, index) {
        ...
        doc.addEventListener('click', e => {
            ...
            if (!a) return
            e.preventDefault()
            /* Берём атрибут href, не свойство a.href: в iframe оно даёт blob:…#… и ломает resolveHref. */
            let href_ = (a.getAttribute('href') || a.getAttributeNS?.('http://www.w3.org/1999/xlink', 'href') || '').trim()
            if (!href_) return
            if (href_.startsWith('blob:')) {
                try {
                    const u = new URL(href_)
                    href_ = u.hash || href_
                } catch { /* keep href_ */ }
            }
            let href = section?.resolveHref?.(href_) ?? href_
            if (href != null && typeof href !== 'string')
                href = href instanceof URL ? href.href : String(href)
    ...
    async initTTS(granularity = 'word', force = false) {
        const first = this.renderer.getContents()[0]
        if (!first?.doc) return
        const doc = first.doc
